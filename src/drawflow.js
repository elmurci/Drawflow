    ...
    if(typenode === false) {
      ...
      if(parseInt(this.render.version) === 3 ) {
        ...
        let wrapper = this.render.createApp({
          parent: this.parent,
          render: h => this.render.h(this.noderegister[html].html, node.props, node.options)
        }).mount(content)
      } else {
        // Vue 2
        let wrapper = new this.render({
          parent: this.parent,
          render: h => h(node.html, { props: node.props }),
          ...node.options
        }).$mount()
        //
  ...
  addNodeImport (dataNode, precanvas) {
    ...
    if(dataNode.typenode === false) {
      ...
      if(parseInt(this.render.version) === 3 ) {
        ...
        node.props["nodeData"] = dataNode;
        let wrapper = this.render.createApp({
          parent: this.parent,
          render: h => this.render.h(node.html, node.props, node.options)
        }).mount(content)
      } else {
        // Vue 2
        let wrapper = new this.render({
